...
class GameControllerSpec extends AnyWordSpec with Matchers with MockitoSugar {
  ...
  /**      "set the game mode and notify observers" in {
            ...
            controller.gameMode shouldBe a[MultiPlayerMode]
            verify(observer, times(2)).update(AskForLoadGame)
            ...
            controller.gameMode shouldBe a[MultiPlayerMode]
            outContent.toString should include("Invalid game mode: x. Defaulting to Multi Player mode.")
        } **/
        ...
        "save the game and notify observers" in {
            val deck = new Deck()
            val hand = new Hand()
            val mockFileIO = mock[FileIOInterface] // Changed to mock the interface instead of concrete class
        ...
        "there are cards in the deck" in {
            ...
            when(deck.drawCard()).thenReturn(Some(testCard)) // Properly mock deck's behavior
        ...
        "there are no cards in the deck" in {
            ...
            controller.currentPlayer shouldBe controller.player1 // Turn should not switch
        }
